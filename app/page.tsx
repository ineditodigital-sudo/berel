...
import { ArrowRight, Check, ChevronDown, ChevronLeft, ChevronRight, CircleUserRound, Headphones, Heart, Menu, Minus, PackageCheck, Plus, Search, ShieldCheck, ShoppingCart, Star, Trash2, Truck, X } from "lucide-react";
...
export default function Home() {
  ...
  const addToCart = useCallback((amount=1) => { setCart(value=>value+amount); notify("Producto agregado al carrito"); }, [notify]);
  const toggleFavorite = useCallback((name:string) => { setFavorites(items => items.includes(name) ? items.filter(item=>item!==name) : [...items,name]); }, []);
  const scrollProducts = useCallback((direction:number) => carouselRef.current?.scrollBy({left:direction*340,behavior:"smooth"}), []);
  useEffect(() => {
  ...
  return <>{splash&&<div className="brand-splash"><div className="splash-logo">berel<small>PINTA CON CONFIANZA</small></div><span/></div>}<main>
    ...
    <section id="asesoria" className="finder"><div><p>ELIGE CON CONFIANZA</p><h2>No todas las pinturas sirven para lo mismo.</h2><span>Cuéntanos qué vas a pintar y te llevamos al producto correcto.</span></div><div className="finder-steps"><button onClick={()=>setFinder(v=>[v[0]==="Muros y plafones"?"Pisos":"Muros y plafones",v[1],v[2]])}><b>1</b><span><small>Superficie</small>{finder[0]}</span><ChevronDown/></button><button onClick={()=>setFinder(v=>[v[0],v[1]==="Interior"?"Exterior":"Interior",v[2]])}><b>2</b><span><small>Ubicación</small>{finder[1]}</span><ChevronDown/></button><button onClick={()=>setFinder(v=>[v[0],v[1],v[2]==="Fácil de limpiar"?"Alta resistencia":"Fácil de limpiar"])}><b>3</b><span><small>Necesidad</small>{finder[2]}</span><ChevronDown/></button><button className="finder-submit" onClick={()=>{setCategory(finder[0]==="Pisos"?"Base agua":"Todos");document.querySelector("#productos")?.scrollIntoView({behavior:"smooth"});notify("Recomendación actualizada")}}>Ver recomendación <ArrowRight/></button></div></section>

    <section id="productos" className="content-section products-section"><div className="section-title"><div><p>PRODUCTOS DESTACADOS</p><h2>Los favoritos de nuestros clientes</h2></div><div className="carousel-actions"><button onClick={()=>scrollProducts(-1)} aria-label="Productos anteriores"><ChevronLeft/></button><button onClick={()=>scrollProducts(1)} aria-label="Productos siguientes"><ChevronRight/></button></div></div>{(query||category!=="Todos") && <div className="query-note">{query?`Resultados para “${query}”`:category} · {visible.length} productos <button onClick={()=>{setQuery("");setCategory("Todos")}}>Limpiar</button></div>}<div className="products-grid carousel" ref={carouselRef}>{visible.map(p=><article className="product-card" key={p.name}><div className="product-image"><span>{p.tag}</span><button className={favorites.includes(p.name)?"favorite active":"favorite"} onClick={()=>toggleFavorite(p.name)} aria-label={`Guardar ${p.name}`}><Heart fill={favorites.includes(p.name)?"currentColor":"none"}/></button><img src={p.image} alt={p.name} loading="lazy" decoding="async"/></div><div className="product-copy"><small>{p.category}</small><h3>{p.name}</h3><div className="stars"><Star fill="currentColor"/> {p.rating} <span>Producto verificado</span></div><div className="product-bottom"><div>{p.old&&<del>{money(p.old)}</del>}<b>{money(p.from)}</b>{p.to&&<small> – {money(p.to)}</small>}</div><button onClick={()=>addToCart()} aria-label={`Agregar ${p.name}`}><Plus/></button></div></div></article>)}</div></section>

    <section className="featured-product"><div className="featured-image"><img src="/berel/pisos.png" alt="Pintura para Pisos Serie 3800" loading="lazy" decoding="async"/></div><div className="featured-copy"><p>PRODUCTO DESTACADO</p><h2>Pintura para Pisos<br/>Serie 3800</h2><p className="desc">Acabado satinado para proteger y decorar pisos de concreto y mortero con tráfico peatonal y vehicular ligero.</p><div className="chips"><span><Check/> Interior y exterior</span><span><Check/> Antideslizante</span><span><Check/> Sin plomo</span><span><Check/> 4–5 m²/L a dos manos</span></div><div className="buy-row"><label>Medida<select value={size} onChange={e=>setSize(e.target.value)}><option>4 L</option><option>19 L</option></select></label><label>Cantidad<div className="qty"><button onClick={()=>setQty(Math.max(1,qty-1))}><Minus/></button><b>{qty}</b><button onClick={()=>setQty(qty+1)}><Plus/></button></div></label></div><div className="featured-price"><div><small>Precio desde</small><b>{money(size==="19 L"?6319:1385)}</b></div><button className="red-button" onClick={()=>addToCart(qty)}>Añadir al carrito <ShoppingCart/></button></div><div className="delivery"><PackageCheck/><span><b>Entrega gratuita</b><small>Disponible en CDMX y Estado de México</small></span></div></div></section>
